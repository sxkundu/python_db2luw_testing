...
class StoreData(PGConnectDB):
    ...
    def retrieve_and_store(self, incomplete_directory, processed_directory):
        ...
        print(self.conn)
        #while True:
        while not self.ins_var_global_fifo_q.empty():
            #try:
            #if not self.ins_var_global_fifo_q.empty():
                #Get data from FIFO queue
                data_from_q = self.ins_var_global_fifo_q.get()
                ...
                if (self.uuid):
                    ...
                    self.dexa_bodycomposition = (self.uuid,
                                                 ...
                                                 self.parsed_result["visceralFat"]["estimatedVisceralAdiposeTissue"][
                                                     "vatMass"]
                                                 )

                    try:
                        self.cursor.execute(self.sqlinsert_bodycomposition, self.dexa_bodycomposition)
                        print("Successfully inserted records dxa.dxatest_bodycomposition")
                    except psycopg2.DatabaseError as error:
                        return (error)
...
class ReadDICOMFiles(PGConnectDB):
    pass
    ...
    def retrieve(self):
        ...
        for file in glob.glob("*.dcm"):
            ...
            if is_dicom(file):
                ...
                if user_id is '':
                    user_id = str(user_firstName) + "." + str(user_lastName) + "." + str(
